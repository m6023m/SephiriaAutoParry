...
$references += (Join-Path $PSScriptRoot 'CombatTrace.cs')
& $compilerPath /nologo /target:library /optimize+ /utf8output /reference:System.Runtime.Serialization.dll ('/out:' + (Join-Path $outputDir 'SephiriaAutoParry.dll')) $references (Join-Path $PSScriptRoot 'AutoParry.cs') (Join-Path $PSScriptRoot 'GuardThreatSelection.cs') (Join-Path $PSScriptRoot 'UpdateCore.cs') (Join-Path $PSScriptRoot 'UpdateUI.cs')
